...
void BTN_Intr_Handler(void *InstancePtr)
{
	...
	if (btn_value == 1) {
		counter_value = 0;// Reset if center button pressed
		xil_printf("reset counter %x\r\n", counter_value);
	}
	else if (btn_value != 0) {
		counter_value++;// Increment counter based on button value
		xil_printf("counter increased %x\r\n", counter_value);
	}
