    ...
    *max_index = 0; // The index of the maximum value in the similarity matrix, initialized to 0
    ...
    for (j = 0; j < M; j++) {
        ...
        for (i = 0; i < N; i++) {
            ...
            north = (j > 0) ? similarity_matrix[index - N] : 0;
            west = (i > 0) ? similarity_matrix[index - 1] : 0;
            northwest = (i > 0 && j > 0) ? similarity_matrix[index - N - 1] : 0;
            ...
            // Check NORTH_WEST
            match_score = (string1[i] == string2[j]) ? MATCH : MISS_MATCH;
...
void print_matrices(int *similarity_matrix, short *direction_matrix, int N, int M, char *query, char *database) {
    ...
    for (int j = 0; j < M; j++) {
        printf("%3c  ", database[j]);
        for (int i = 0; i < N; i++) {
            printf("%3d ", similarity_matrix[j * N + i]);
        }
        printf("\n");
    }
    ...
    for (int j = 0; j < M; j++) {
        printf("%3c  ", database[j]);
        for (int i = 0; i < N; i++) {
            printf("%3s ", dir_symbols[direction_matrix[j * N + i]]);
        }
        printf("\n");
    }
}
...
/* Create the two input strings by calling a random number generator */
    ...
    // printf("cnt_ops=%d, cnt_bytes=oz%d\n", cnt_ops, cnt_bytes);
	return EXIT_SUCCESS;
}
