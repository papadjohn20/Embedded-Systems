...
short *order_matrix_blocks(ap_uint<128> *in_matrix_128) { // change to ap_uint<128>
    ...
    for (int k = 2; k <= (N + M); k++) {
		...
		ap_uint<128> packed_diagonal = in_matrix_128[k - 2]; // 128 bits
        ...
        for (int i = 1; i <= N; i++) {
			...
			if (j >= 1 && j <= M && i >= 1 && i <= N) {
                short value = packed_diagonal((i - 1) * 2 + 1, (i - 1) * 2); 
...
int main(int argc, char** argv) {
	...
	// The size of the kernel's matrix (128 bit blocks)
	// Every diagonal has 128 bits. Diagonals = M + N - 1
	int total_diagonals = M + N - 1;
    size_t matrix_bytes = total_diagonals * sizeof(ap_uint<128>); // 64 -> 128

	char *query = (char*) malloc(sizeof(char) * N);
	char *database = (char*) malloc(sizeof(char) * M);

	ap_uint<128> *direction_matrix = (ap_uint<128>*) malloc(matrix_bytes);
	...
	size_t query_bytes = 1 * sizeof(ap_uint<512>);
	ap_uint<512> *encoded_query = (ap_uint<512>*) calloc(1, sizeof(ap_uint<512>));

	// Database (M=65536) is packed for each 170 characters in a 512-bit block
	int s2_blocks_count = (M + 169) / 170;
	size_t database_bytes = s2_blocks_count * sizeof(ap_uint<512>);
	ap_uint<512> *encoded_database = (ap_uint<512>*) calloc(s2_blocks_count, sizeof(ap_uint<512>));

    // Packing Query
    for (int i = 0; i < N; i++) {
        ap_uint<3> enc = encode_base(query[i]);
        encoded_query[0]((i * 3) + 2, i * 3) = enc;
    }

    
    // Packing Database
    for (int j = 0; j < M; j++) {
        ap_uint<3> enc = encode_base(database[j]);
        int block_idx = j / 170; // 170 characters per 512-bit block
        int bit_offset = (j % 170) * 3;
        encoded_database[block_idx](bit_offset + 2, bit_offset) = enc;
    }
 ...
 * *********************************************/
  ...
  /********************************************************
    ...
    **************************************************************/
	input_query = clCreateBuffer(context, CL_MEM_READ_ONLY, query_bytes,
	NULL, NULL);
	input_database = clCreateBuffer(context, CL_MEM_READ_ONLY, database_bytes,
	NULL, NULL);
    ...
    **************************************************************/
	err = clEnqueueWriteBuffer(commands, input_query, CL_TRUE, 0,
			query_bytes, encoded_query, 0, NULL, NULL);
	if (err != CL_SUCCESS) {
	...
	err = clEnqueueWriteBuffer(commands, input_database, CL_TRUE, 0,
			database_bytes, encoded_database, 0, NULL, NULL);
	if (err != CL_SUCCESS) {
		printf("Error: Failed to write to source array a!\n");
		printf("Test failed\n");
		return EXIT_FAILURE;
 ...
 **************************************************************/
	...
	printf("both ended 64 125000 find max\n");
