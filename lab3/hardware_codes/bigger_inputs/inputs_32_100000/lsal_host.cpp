...
short *order_matrix_blocks(ap_uint<64> *in_matrix_64) {
    ...
    for (int k = 2; k <= (N + M); k++) {
        ...
        ap_uint<64> packed_diagonal = in_matrix_64[k - 2];
...
int main(int argc, char** argv) {
	...
	// The size of the kernel's matrix (64 bit blocks)
	// Every diagonal has 64 bits. Diagonals = M + N - 1
	int total_diagonals = M + N - 1;
    size_t matrix_bytes = total_diagonals * sizeof(ap_uint<64>);

	char *query = (char*) malloc(sizeof(char) * N);
	char *database = (char*) malloc(sizeof(char) * M);

	ap_uint<64> *direction_matrix = (ap_uint<64>*) malloc(matrix_bytes);
 ...
 **************************************************************/
	...
	printf("both ended final opt 32 100000\n");
