...
short *order_matrix_blocks(ap_uint<512> *in_matrix_512) {
    // Cast the 512-bit hardware pointer to short* to unpack the 16-bit elements
    short *in_matrix = (short *)in_matrix_512;

    // Allocate the 2D output matrix
    short *out_matrix = (short *)malloc(sizeof(short) * (N + 1) * (M + 1));

    // Set the matrix with zeros
    memset(out_matrix, 0, sizeof(short) * (N + 1) * (M + 1));

    // Reconstruct using the correct hardware diagonal streaming step (always 32 elements)
    for (int k = 2; k <= (N + M); k++) {
        int diag_start_idx = (k - 2) * 32; // The FPGA always writes 32 shorts per diagonal

        for (int i = 1; i <= N; i++) {
            int j = k - i;

            // Bounds check for our matrix dimensions
            if (j >= 1 && j <= M && i >= 1 && i <= N) {
                short value = in_matrix[diag_start_idx + (i - 1)];
                int store_index = j * (N + 1) + i;
                out_matrix[store_index] = value;
            }
        }
    }
...
inline ap_uint<3> encode_base (char c) {
    switch(c) {
...
int main(int argc, char** argv) {
	...
	// The size of the kernel's matrixes (512 bit blocks)
	// Every diagonal has 32 shorts = 512 bits. Diagonals = M + N - 1
	int total_diagonals = M + N - 1;
    size_t matrix_bytes = total_diagonals * sizeof(ap_uint<512>);

	char *query = (char*) malloc(sizeof(char) * N);
	char *database = (char*) malloc(sizeof(char) * M);

    ap_uint<512> *similarity_matrix = (ap_uint<512>*) malloc(matrix_bytes);
	ap_uint<512> *direction_matrix = (ap_uint<512>*) malloc(matrix_bytes);
	...
	// Encode the inputs with ap_uint<3>
	ap_uint<3> *encoded_query = (ap_uint<3>*) malloc(sizeof(ap_uint<3>) * N);
    ap_uint<3> *encoded_database = (ap_uint<3>*) malloc(sizeof(ap_uint<3>) * M);

    for (int i = 0; i < N; i++) {
        encoded_query[i] = encode_base(query[i]);
    }
    for (int j = 0; j < M; j++) {
        encoded_database[j] = encode_base(database[j]);
    }

    memset(similarity_matrix, 0, matrix_bytes);
	memset(direction_matrix, 0, matrix_bytes);
 ...
 * *********************************************/
  ...
  /********************************************************
    ...
    **************************************************************/
	input_query = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(ap_uint<3>) * N,
	NULL, NULL);
	input_database = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(ap_uint<3>) * M,
	NULL, NULL);
    output_similarity_matrix = clCreateBuffer(context, CL_MEM_READ_WRITE, matrix_bytes,
    NULL, NULL);
	output_direction_matrix = clCreateBuffer(context, CL_MEM_READ_WRITE, matrix_bytes,
	NULL, NULL);
	output_max_index = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int),
	NULL, NULL);

	if (!input_query || !input_database || !output_similarity_matrix || !output_direction_matrix || !output_max_index) {
    ...
    **************************************************************/
	err = clEnqueueWriteBuffer(commands, input_query, CL_TRUE, 0,
			sizeof(ap_uint<3>) * N, encoded_query, 0, NULL, NULL);
	if (err != CL_SUCCESS) {
	...
	err = clEnqueueWriteBuffer(commands, input_database, CL_TRUE, 0,
			sizeof(ap_uint<3>) * M, encoded_database, 0, NULL, NULL);
	if (err != CL_SUCCESS) {
		printf("Error: Failed to write to source array a!\n");
		printf("Test failed\n");
		return EXIT_FAILURE;
    ...
    err = clEnqueueWriteBuffer(commands, output_similarity_matrix, CL_TRUE, 0, matrix_bytes,
	...
	err = clEnqueueWriteBuffer(commands, output_direction_matrix, CL_TRUE, 0, matrix_bytes,
    ...
    err = clEnqueueReadBuffer(commands, output_similarity_matrix, CL_TRUE, 0,
			matrix_bytes, similarity_matrix, 0, NULL, &readSimilarity);
	if (err != CL_SUCCESS) {
	...
	err = clEnqueueReadBuffer(commands, output_direction_matrix, CL_TRUE, 0,
			matrix_bytes, direction_matrix, 0, NULL, &readDirections);
	if (err != CL_SUCCESS) {
		printf("Error: Failed to read array! %d\n", err);
		printf("Test failed\n");
		return EXIT_FAILURE;
 ...
 **************************************************************/
	...
	printf("both ended second diag opt\n");
	printf("execution time is %lf ms \n", executionTime);


	short *ordered_direction_matrix = order_matrix_blocks(direction_matrix);
	...
	for (int row = 0; row <= M; row++) {
		for (int col = 0; col <= N; col++) {
			...
			if (direction_matrix_sw[i] != ordered_direction_matrix[i]) {
				errors++;
				if (errors < 5) {
					printf("Direction Mismatch at row %d, col %d (index %d): HW=%d, SW=%d\n", 
						row, col, i, ordered_direction_matrix[i], direction_matrix_sw[i]);
				}
			}
		}
	}
