...
short *order_matrix_blocks(short *in_matrix) {
    // Allocate the 2D output matrix with (N + 1) * (M + 1) elements
    short *out_matrix = (short *)malloc(sizeof(short) * (N + 1) * (M + 1));

    // Clear the matrix with zeros
    ...
    for (int k = 2; k <= (N + M); k++) {
        int diag_start_idx = (k - 2) * N; // The FPGA always writes N elements per diagonal
...
int main(int argc, char** argv) {
    ...
    size_t similarity_bytes = total_diagonals * N * sizeof(int);
    size_t directional_bytes = total_diagonals * N * sizeof(short);
 ...
 * *********************************************/
  ...
  /********************************************************
    ...
    **************************************************************/
	...
	printf("both ended first diag opt\n");
	printf("execution time is %lf ms \n", executionTime);

	// Reorder the matrix to a 2D shape
